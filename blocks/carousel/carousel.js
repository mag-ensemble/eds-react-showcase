...
function createSlide(row, slideIndex, carouselId) {
  ...
  const pictureDiv = createOptimizedPicture(image, title, false, [{ width: '750' }]);
...
export default async function decorate(block) {
  ...
  data.forEach((row, idx) => {
    ...
    if (slideIndicators) {
      ...
      indicator.innerHTML = `<button type="button" aria-label="${placeholders.showSlide || 'Show Slide'} ${idx + 1} ${placeholders.of || 'of'} ${data.length}"></button>`;
